...
import { cloneJson, compare, fileExists, formatMetadata, generateImageTiles, getFileDimensions, getFormatByExtension, getFormatByExtensionAndType, getFormatByType, getLabel, getThumbnail, getTypeByExtension, getTypeByFormat, isDirectory, isURL, log, normaliseFilePath, normaliseType, readYml, warn, } from "./Utils.js";
...
export class Canvas {
    constructor(filePath, parentDirectory) {
        ...
        this.directory = new Directory(this.directoryFilePath, this.parentDirectory.url.href, undefined, this.parentDirectory);
    ...
    async read(canvasJson) {
        ...
        if (this._isCanvasDirectory()) {
            ...
            const customAnnotationFiles = await glob(this.directoryFilePath + "/*.yml", {
                ignore: ["**/info.yml"],
            });
            // sort files
            ...
            await Promise.all(customAnnotationFiles.map(async (file) => {
                ...
                const annotationJson = cloneJson(annotationBoilerplate);
                const yml = await readYml(file);
                annotationJson.id = urljoin(canvasJson.id, "annotation", String(canvasJson.items[0].items.length));
                ...
                if (yml.type) {
                    ...
                    const type = getTypeByExtension(motivation, extname(yml.value));
                    if (type) {
                    ...
                    const type = getTypeByFormat(motivation, yml.format);
                    if (type) {
                        annotationJson.body.type = type;
                        warn(`type property missing in ${file}, guessed ${type}`);
                    }
                ...
                if (yml.format) {
                    ...
                    const format = getFormatByExtensionAndType(motivation, extname(yml.value), yml.type);
                    if (format) {
                    ...
                    const format = getFormatByExtension(motivation, extname(yml.value));
                    if (format) {
                        annotationJson.body.format = format;
                        warn(`format property missing in ${file}, guessed ${format}`);
                    }
                    ...
                    const format = getFormatByType(motivation, yml.type);
                    if (format) {
                        annotationJson.body.format = format;
                        warn(`format property missing in ${file}, guessed ${format}`);
                    }
                ...
                if (annotationJson.body.type &&
                    ...
                    const service = cloneJson(imageServiceBoilerplate);
                ...
                if (yml.value && !isURL(yml.value) && annotationJson.body.type) {
                    ...
                    let path = join(dirName, yml.value);
                    path = normaliseFilePath(path);
                    await getFileDimensions(annotationJson.body.type, path, canvasJson, annotationJson);
                }
    ...
    async _annotateFiles(canvasJson, files) {
        await Promise.all(files.map(async (file) => {
            ...
            if (defaultPaintingExtension) {
                ...
                const annotationJson = cloneJson(annotationBoilerplate);
                annotationJson.id = urljoin(canvasJson.id, "annotation", String(canvasJson.items[0].items.length));
                annotationJson.motivation = normaliseType(AnnotationMotivation.PAINTING);
                annotationJson.target = canvasJson.id;
                annotationJson.body.id = id;
                ...
                annotationJson.body.label = getLabel(this.infoYml.label);
                canvasJson.items[0].items.push(annotationJson);
                await getFileDimensions(defaultPaintingExtension.type, file, canvasJson, annotationJson);
                const hocrFiles = await glob(this.directoryFilePath + "/*.hocr");
                if (hocrFiles) {
                    ...
                    if (hocrFile) {
                        ...
                        const hocrFileName = basename(hocrFile);
                        const hocrId = urljoin(this.url.href, directoryName, hocrFileName);
                        const hocrJson = cloneJson(hocrBoilerplate);
                        hocrJson["@id"] = hocrId;
                ...
                if (defaultPaintingExtension.type.toLowerCase() ===
                    ExternalResourceType.IMAGE) {
                    await generateImageTiles(file, this.url.href, directoryName, this.directoryFilePath, annotationJson);
                }
            }
        }));
    }
